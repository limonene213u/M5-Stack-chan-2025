...
String current_message = "Standby...";
...
void bluetoothCallback(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) {
  if (event == ESP_SPP_SRV_OPEN_EVT) {
    ...
    current_message = "Bluetooth connected";
    avatar.setSpeechText(current_message.c_str());
    M5_LOGI("Bluetooth connected");
  } else if (event == ESP_SPP_CLOSE_EVT) {
    bluetooth_connected = false;
    current_message = "Bluetooth disconnected";
    avatar.setSpeechText(current_message.c_str());
...
// 初期化とメイン処理
void setupWiFi() {
  WiFi.begin(comm_config.wifi_ssid.c_str(), comm_config.wifi_password.c_str());
  current_message = "WiFi connecting...";
  avatar.setSpeechText(current_message.c_str());
  ...
  if (WiFi.status() == WL_CONNECTED) {
    current_message = "WiFi OK: " + WiFi.localIP().toString();
    M5_LOGI("WiFi connected! IP: %s", WiFi.localIP().toString().c_str());
  } else {
    current_message = "WiFi Failed";
...
void setupBluetooth() {
  ...
  current_message = "Bluetooth ready...";
  avatar.setSpeechText(current_message.c_str());
...
void setup() {
  ...
  // Webサーバーポート設定
  server = WebServer(comm_config.webserver_port);
...
void loop() {
  ...
  if (M5.BtnA.wasPressed()) {
    ...
    switch (current_mode) {
      case WIFI_MODE:
        ...
        current_message = "Bluetooth Mode";
        break;
      case BLUETOOTH_MODE:
        current_mode = BOTH_MODE;
        current_message = "Both Mode";
        break;
      case BOTH_MODE:
        current_mode = WIFI_MODE;
        current_message = "WiFi Mode";
        break;
    }
    avatar.setSpeechText(current_message.c_str());
  ...
  if (M5.BtnB.wasPressed()) {
    // ステータス表示
    std::string statusStr = formatString("IP:%s BT:%s", 
                                WiFi.localIP().toString().c_str(),
                                bluetooth_connected ? "Connected" : "Disconnected");
    current_message = String(statusStr.c_str());
    avatar.setSpeechText(current_message.c_str());
  }
  ...
  if (M5.BtnC.wasPressed()) {
    ...
    current_message = "Expression changed!";
    avatar.setSpeechText(current_message.c_str());
  }
