...
const char* WIFI_SSID = "YOUR_WIFI_SSID";        // ここにあなたのWiFi名を入力
const char* WIFI_PASSWORD = "YOUR_WIFI_PASSWORD"; // ここにパスワードを入力
...
// WiFi Setup with fallback options
void setupWiFi() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nPrevious connection failed. Trying configured network...");
    
    // Try configured WiFi first
    if (strlen(WIFI_SSID) > 0 && strcmp(WIFI_SSID, "YOUR_WIFI_SSID") != 0) {
      Serial.printf("Trying configured network: %s\n", WIFI_SSID);
    ...
    // Try some common network names if configured network fails
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("\nTrying common networks...");
      const char* common_ssids[] = {"iPhone", "AndroidAP", "HUAWEI", "Galaxy"};
      const char* common_passwords[] = {"12345678", "password", "12345678", "12345678"};
      
      for (int i = 0; i < 4; i++) {
        Serial.printf("Trying %s...\n", common_ssids[i]);
        WiFi.begin(common_ssids[i], common_passwords[i]);
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    M5.Display.println("SmartConfig Mode");
    M5.Display.println("Use smartphone");
    M5.Display.println("WiFi settings");
    M5.Display.println("or press button C");
    M5.Display.println("to skip");
    
    WiFi.beginSmartConfig();
    
    unsigned long smartconfig_start = millis();
    while (!WiFi.smartConfigDone() && (millis() - smartconfig_start) < 60000) {  // 60秒タイムアウト
      delay(500);
      Serial.print("#");
      
      // Check for button press to skip SmartConfig
      M5.update();
      if (M5.BtnC.wasPressed()) {
        Serial.println("\nSmartConfig skipped by user");
    ...
    if (WiFi.smartConfigDone()) {
      ...
      M5.Display.clear();
      M5.Display.setCursor(0, 0);
      M5.Display.println("SmartConfig完了");
      ...
      Serial.println("\nSmartConfig timeout or skipped");
      M5.Display.clear();
      M5.Display.setCursor(0, 0);
      M5.Display.println("WiFi not connected");
      M5.Display.println("Starting AP mode");
      
      // Start AP mode as fallback
      WiFi.mode(WIFI_AP);
      WiFi.softAP("Stack-chan-Setup", "stack-chan-88");
      Serial.println("AP Mode started: Stack-chan-Setup / stack-chan-88");
      M5.Display.println("AP: Stack-chan-Setup");
      M5.Display.println("Pass: stack-chan-88");
      return;
    }
  }
...
void setup() {
  ...
  // Basic system configuration (simplified for communication edition)
  Serial.println("Using default communication configuration");
