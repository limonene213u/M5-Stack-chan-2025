...
<body>
    <div class="container">
        <h1>🤖 Stack-chan Communication Panel</h1>
        
        <div class="input-group">
            <label for="message">メッセージを送信:</label>
            <textarea id="message" rows="3" placeholder="表示したいメッセージを入力してください"></textarea>
            <button class="button" onclick="sendMessage()">送信</button>
        </div>
        
        <div class="input-group">
            <label for="expression">表情を変更:</label>
            <select id="expression">
                <option value="normal">普通</option>
                <option value="happy">嬉しい</option>
                <option value="sleepy">眠い</option>
                <option value="doubt">疑問</option>
            </select>
            <button class="button" onclick="changeExpression()">変更</button>
        </div>
    ...
    <script>
        function sendMessage() {
            ...
            .then(data => {
                alert('メッセージが送信されました: ' + data.status);
        ...
        function updateStatus() {
            ...
            .then(data => {
                document.getElementById('status').innerHTML = `
                    <p><strong>現在のメッセージ:</strong> ${data.current_message}</p>
                    <p><strong>最後の受信データ:</strong> ${data.last_received}</p>
                    <p><strong>Bluetooth接続:</strong> ${data.bluetooth_connected ? '接続済み' : '未接続'}</p>
                    <p><strong>WiFi接続:</strong> ${data.wifi_connected ? '接続済み' : '未接続'}</p>
                    <p><strong>IPアドレス:</strong> ${data.ip_address}</p>
                `;
            });
        }
        
        // ページ読み込み時とその後5秒間隔で状態更新
        updateStatus();
