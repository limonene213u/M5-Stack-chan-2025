...
/**
 * Avatar初期化モジュール（独立・順次処理）
 * 他のタスクが開始する前に完全に初期化を完了
 */
bool initializeAvatarModule() {
  Serial.println("DEBUG: Avatar module initialization started");
  ...
  try {
    ...
    Serial.println("DEBUG: Avatar module initialized successfully (sequential)");
    return true;
    
  } catch (...) {
    Serial.println("ERROR: Avatar module initialization failed");
...
void setup() {
  ...
  // インオーダー初期化アプローチ：モジュール順次完了
  Serial.println("DEBUG: Starting sequential module initialization");
  M5.Display.fillScreen(TFT_BLACK);
  M5.Display.setTextSize(1);
  M5.Display.setTextColor(TFT_WHITE);
  M5.Display.setCursor(10, 10);
  M5.Display.print("Stack-chan Sequential Init");
  
  // Step 1: Avatar初期化を最優先（他のタスク開始前）
  Serial.println("DEBUG: Avatar initialization FIRST (before any tasks)");
  M5.Display.setCursor(10, 30);
  M5.Display.print("Step 1: Avatar init...");
  
  if (initializeAvatarModule()) {
    M5.Display.setCursor(10, 50);
    M5.Display.print("Step 1: Avatar OK");
    Serial.println("DEBUG: Avatar module initialized successfully");
  } else {
    M5.Display.setCursor(10, 50);
    M5.Display.print("Step 1: Avatar SKIP");
    Serial.println("DEBUG: Avatar module skipped");
  }
  delay(1000);  // Avatar安定化待機
  
  // Step 2: WiFi初期化（Avatar完了後）
  Serial.println("DEBUG: WiFi initialization AFTER Avatar stabilization");
  M5.Display.setCursor(10, 70);
  M5.Display.print("Step 2: WiFi setup...");
  setupWiFi();
  M5.Display.setCursor(10, 90);
  M5.Display.print("Step 2: WiFi OK");
  delay(500);  // WiFi安定化待機
  
  // Step 3: WebServer初期化（WiFi完了後）
  Serial.println("DEBUG: WebServer initialization AFTER WiFi stabilization");
  M5.Display.setCursor(10, 110);
  M5.Display.print("Step 3: WebServer...");
  setupWebServer();
  M5.Display.setCursor(10, 130);
  M5.Display.print("Step 3: WebServer OK");
  
  // 全モジュール初期化完了
  M5.Display.setCursor(10, 150);
  M5.Display.print("Sequential init COMPLETE!");
  Serial.println("DEBUG: All modules initialized in sequence");
...
void loop() {
  ...
  if (!avatar_initialized && avatar_init_start > 0 && (millis() - avatar_init_start) > 5000) {
    ...
    try {
      ...
      // Avatar初期化（段階的・安全アプローチ - StackchanSystemConfig復活版）
      avatar.init();
      avatar.setColorPalette(*cps[0]);
      avatar.setSpeechFont(&fonts::efontJA_16);  // 日本語フォント設定
      avatar.setExpression(Expression::Neutral);
      ...
      // 内蔵タスクのみ使用（外部関数依存を回避）
      // face, lipSync等の外部関数は存在しないため、内蔵機能のみでAvatar表示
