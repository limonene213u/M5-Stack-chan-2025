...
void handleMessage() {
  ...
  drawCustomString(5, M5.Display.height() - 38, "WiFi受信:", TFT_GREEN);
  drawCustomString(5, M5.Display.height() - 18, message.c_str(), TFT_WHITE);
      ...
      if (receivedData.startsWith("{")) {
        ...
        if (!error) {
          ...
          if (doc.containsKey("message")) {
            ...
            // カスタム日本語フォントで画面表示
            M5.Display.fillRect(0, M5.Display.height() - 40, M5.Display.width(), 40, TFT_BLACK);
            drawCustomString(5, M5.Display.height() - 38, "BT受信:", TFT_BLUE);
            drawCustomString(5, M5.Display.height() - 18, current_message.c_str(), TFT_WHITE);
          }
          ...
          // カスタム日本語フォントで画面表示
          M5.Display.fillRect(0, M5.Display.height() - 40, M5.Display.width(), 40, TFT_BLACK);
          drawMixedString(5, M5.Display.height() - 38, "BT受信:", TFT_BLUE);
          drawMixedString(5, M5.Display.height() - 18, current_message.c_str(), TFT_WHITE);
        }
      } else {
        // プレーンテキストの場合
        current_message = receivedData;
...
void setup() {
  ...
  drawMixedString(10, 10, "カスタム日本語フォント", TFT_GREEN);
  drawMixedString(10, 30, "初期化中...", TFT_WHITE);
  delay(1000);
...
void loop() {
  ...
  if ((millis() - last_display_update) > display_update_interval) {
    ...
    if (comm_config.lyrics.size() > 0) {
      ...
      // 画面下部にカスタムフォントでメッセージ表示
      M5.Display.fillRect(0, M5.Display.height() - 20, M5.Display.width(), 20, TFT_BLACK);
      drawMixedString(5, M5.Display.height() - 18, message.c_str(), TFT_WHITE);
    }
