...
// 安全なフォント設定関数
bool setJapaneseFont() {
  // M5GFXライブラリの初期化状態をチェック
  if (!M5.Display.isReady()) {
    return false;
  }
  
  // より安全な方法：小さいフォントから試行
  M5.Display.setFont(&fonts::lgfxJapanGothic_12);  // より軽いフォント
  delay(10);  // フォント設定の安定化待ち
...
void handleMessage() {
  ...
  // フォント設定を試行、失敗時はデフォルトフォントを使用
  bool japanese_font_ok = true;
  try {
    M5.Display.setFont(&fonts::efontJA_16);
  } catch (...) {
    M5.Display.setFont(); // デフォルトフォント
    japanese_font_ok = false;
  }
...
void setup() {
  ...
  // シンプルな初期化メッセージ（英語フォントで安全に表示）
  M5.Display.fillScreen(TFT_BLACK);
  M5.Display.setTextSize(2);
  M5.Display.setTextColor(TFT_GREEN);
  M5.Display.setCursor(10, 10);
  M5.Display.print("Stack-chan");
  M5.Display.setTextColor(TFT_WHITE);
