...
void BLEWebUIHandler::begin() {
    ...
    // 発見しやすくするための設定
    pAdvertising->setScanResponse(true);  // スキャンレスポンスを有効化
    pAdvertising->setMinPreferred(0x06);  // 最小接続間隔を短く
    pAdvertising->setMaxPreferred(0x12);  // 最大接続間隔を設定
    
    // アドバタイズメント間隔を短く（発見しやすくする）
    pAdvertising->setAdvertisementInterval(100); // 100ms間隔（デフォルトは1280ms）
    ...
    scanResponseData.setShortName("StackChan");
    ...
    Serial.println("BLEペアリングモードで待機中...");
