...
void setup() {
  ...
  // 単純なテスト表示（Avatar完全スキップ）
  Serial.println("DEBUG: Starting simple test without Avatar");
  M5.Display.fillScreen(TFT_BLACK);
  M5.Display.setTextSize(2);
  M5.Display.setTextColor(TFT_WHITE);
  M5.Display.setCursor(10, 50);
  M5.Display.print("Stack-chan Test");
  M5.Display.setCursor(10, 100);
  M5.Display.print("Simple Mode");
  Serial.println("DEBUG: Simple test completed");
  
  Serial.println("DEBUG: Entering loop");
  M5.Display.setCursor(10, 90);
  M5.Display.print("Step 4: Avatar Already Init");
  
  // 単純なWiFi設定（Avatar機能なし）
