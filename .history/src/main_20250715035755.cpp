...
void setup() {
  ...
  // 🚨 COMPLETE AVATAR DISABLE: FreeRTOSキュー競合回避のため一時的に全Avatar機能を無効化
  Serial.println("DEBUG: Avatar initialization DISABLED for stability");
  
  // Avatar初期化を完全無効化（FreeRTOSキュー作成を回避）
  /*
  avatar.init();
  ...
  */
  ...
  // 通信エディション特化：Avatar完全無効化
  Serial.println("DEBUG: Avatar initialization SKIPPED for FreeRTOS stability");
  
  avatar_initialized = false;  // 初期化無効フラグ
